...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMBloomShaker : MonoBehaviour
    {
        ...
        protected virtual void Shake()
        {
            ...
            if (RelativeIntensity) { _bloom.intensity.Override(_bloom.intensity.value + _initialIntensity); }

#if URP
            _bloom.threshold.Override(ShakeThreshold.Evaluate(_remappedTimeSinceStart) * ShakeThresholdAmplitude);
            if (RelativeIntensity) { _bloom.threshold.Override(_bloom.threshold.value + _initialThreshold); }
#endif
        }
