...
public struct OverlappingTriggerVolume : IBufferElementData
{
...
unsafe public class TriggerVolumeSystem : JobComponentSystem
{
    ...
    struct UpdateOverlappingJob : IJob
    {
        ...
        public BufferFromEntity<OverlappingTriggerVolume> OverlappingGroup;
    ...
    struct AddNewOverlappingJob : IJob
    {
        ...
        public void Execute()
        {
            for (int i = 0; i < TriggerEntitiesCount[0]; i++)
            {
                ...
                // frame.
                DynamicBuffer<OverlappingTriggerVolume> buf;
                if (!OverlappingGroup.Exists(overlappingEntity)) {
                    buf = CommandBuffer.AddBuffer<OverlappingTriggerVolume>(overlappingEntity);
                } else
                {
                    buf = OverlappingGroup[overlappingEntity];
                }
                //Debug.Log($"entities.VolumeEntity {entities.VolumeEntity}");

                if (!TriggerGroup.Exists(entities.VolumeEntity))
                {   // TriggerVolume has already deleted, skip
                    continue;
                }
                ...
                // TODO: use NativeHashMap instead;
                for (int idx = 0; idx < buf.Length; idx++)
                {
                    var item = buf[idx];
                    if (item.VolumeEntity == entities.VolumeEntity)
                    {
                        buf.RemoveAt(idx);
                    }
                }
                buf.Add(overlapComponent);
    ...
    struct RemoveOldOverlappingJob : IJob
    {
        ...
        public void Execute()
        {
            for (int index = 0; index < OverlappingEntities.Length; index++)
            {
                ...
                //Debug.Log($"remove old Overlapping {entity}");
                var overlappingBuffer = OverlappingGroup[entity];
                
                for (int i = 0; i < overlappingBuffer.Length; i++)
                {
                    var overlappingComponent = overlappingBuffer[i];
                    if (!TriggerGroup.Exists(overlappingComponent.VolumeEntity))
                    { // TriggerVolume has already deleted.
                        overlappingBuffer.RemoveAt(i);
                        continue;
                    }
    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        var overlappingGroup = GetBufferFromEntity<OverlappingTriggerVolume>(); // GetComponentDataFromEntity<OverlappingTriggerVolume>();
